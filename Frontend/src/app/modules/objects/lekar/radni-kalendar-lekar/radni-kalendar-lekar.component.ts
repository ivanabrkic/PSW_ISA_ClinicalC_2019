...
import {DijalogKreiranjeReceptaComponent} from '../../../shared/dijalog-kreiranje-recepta/dijalog-kreiranje-recepta.component';
...
export class RadniKalendarLekarComponent implements OnInit {
    ...
    this.pregledService.getPredefinisane(this.sessionService.ulogovanLekar.jbo).subscribe(pregledi => {
      ...
      if (this.pregledi) {
        ...
        for (i = 0; i < this.pregledi.length; i++) {
          ...
          console.log(this.pregledi[i].jboPacijenta);
  ...
  zapocniPregled(pacijentJbo: any, startTime: Date, endTime: Date, idPregleda: number) {
    ...
    const pregled = new PregledIzvestajDTO(idPregleda, pacijentJbo, datum, pocetak, kraj,
      this.sessionService.ulogovanLekar.jbo, this.sessionService.ulogovanLekar.ime, this.sessionService.ulogovanLekar.prezime);
